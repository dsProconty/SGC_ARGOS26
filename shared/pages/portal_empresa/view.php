...
function cargarNomina(buscar) {
    ...
    $('#tbody_nomina').html('<tr><td colspan="8" class="text-center py-3"><i class="icon dripicons-loading icon-spin"></i> Cargando...</td></tr>');
    $.getJSON(url, function(r) {
        if (!r.success) { $('#tbody_nomina').html('<tr><td colspan="8" class="text-center text-danger">' + r.mensaje + '</td></tr>'); return; }
        nominaData = r.data;
        var html = '';
        if (!r.data.length) {
            html = '<tr><td colspan="8" class="text-center text-muted py-3">Sin resultados</td></tr>';
        } else {
            r.data.forEach(function(e) {
                var pct = e.per_cupo_asignado > 0 ? (parseFloat(e.consumido) / parseFloat(e.per_cupo_asignado) * 100).toFixed(0) : 0;
