...
if (!function_exists('ec_modo_preproduccion')) {
    /**
     * Interruptor de seguridad único para los 3 caminos de envío (botón
     * manual "Enviar", cron/enviar_estados_cuenta.php, y el piggyback en
     * content.php): mientras esté activo, ec_enviar_correo() nunca manda un
     * correo real, sin importar cuál de los tres lo haya disparado.
     *
     * Default "cerrado" (bloquea el envío) si la fila todavía no existe en
     * `configuracion` — mejor pecar de cauto que mandar un correo real por
     * accidente mientras se está probando el sistema. Para pasar a
     * producción real: UPDATE configuracion SET cfg_valor='0' WHERE
     * cfg_clave='modo_preproduccion'.
     */
    function ec_modo_preproduccion($mysqli) {
        $r    = mysqli_query($mysqli, "SELECT cfg_valor FROM configuracion WHERE cfg_clave = 'modo_preproduccion'");
        $fila = $r ? mysqli_fetch_assoc($r) : null;
        return $fila === null || $fila['cfg_valor'] === '1';
    }
}
...
if (!function_exists('ec_enviar_correo')) {
    ...
    function ec_enviar_correo($mysqli, $ec_id) {
        ...
        if (ec_modo_preproduccion($mysqli)) {
            return array(
                'success' => false,
                'mensaje' => 'Modo preproducción activo: el envío de correos está desactivado en Configuración.',
            );
        }
        ...
        if (!$email) {
            mysqli_query($mysqli, "UPDATE estado_cuenta SET ec_estado_envio = 'error', ec_error_detalle = 'Sin correo registrado' WHERE ec_id = $ec_id");
        ...
        if ($enviado) {
            mysqli_query($mysqli, "UPDATE estado_cuenta SET ec_estado_envio = 'enviado', ec_fecha_envio = NOW(), ec_error_detalle = NULL WHERE ec_id = $ec_id");
...
if (!function_exists('ec_generar_y_enviar_automaticos')) {
     ...
     *
     * El bloqueo por modo_preproduccion vive dentro de ec_enviar_correo(),
     * no acá: así este disparador sigue generando el registro del estado de
     * cuenta con normalidad (útil para revisar montos/pruebas), y solo el
     * envío del correo queda contenido mientras el modo esté activo.
     */
